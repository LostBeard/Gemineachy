...
// TEMP DIAGNOSTIC (remove once the cold-start wake question is settled): record every service-worker
// startup, so "Chrome never woke the worker" can be told apart from "the worker woke and something
// downstream failed". Polling chrome://json for a service_worker target cannot distinguish those - a fast
// start-then-die slips between polls.
try {
    chrome.storage.local.get({ swStarts: [] }, function (r) {
        var a = (r && r.swStarts) || [];
        a.push(new Date().toISOString());
        if (a.length > 30) a = a.slice(-30);
        chrome.storage.local.set({ swStarts: a });
    });
} catch (e) { }

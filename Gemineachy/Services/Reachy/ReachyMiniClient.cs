// COPIED VERBATIM from D:\users\tj\Projects\SpawnDev.Reachy\SpawnDev.Reachy\ReachyMiniClient.cs (2026-08-15).
// Reused unchanged in the browser extension: it is plain HttpClient + source-gen JSON, so it runs under
// Blazor WASM. In the extension the HttpClient is given a BackgroundRelayHttpHandler so each request is
// executed by the background service worker (which can fetch the LAN daemon cross-origin; the page cannot).
